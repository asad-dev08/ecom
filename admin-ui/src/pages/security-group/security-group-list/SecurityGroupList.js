...
import CreateSecurityGroup from "../create-security-group/CreateSecurityGroup";
import {
  deleteSecurityGroup,
  getSecurityGroup,
...
const SecurityGroupList = () => {
  ...
  const actionHandlers = {
    ...
    handleDeleteModalOk: async () => {
      const response = await dispatch(
        deleteSecurityGroup(state.selectedRow.id)
      );
      if (response) {
  ...
  const handleSecurityGroupChange = useCallback(() => {
    ...
    <div>
      ...
      {state.open && (
        <CreateSecurityGroup
          ...
          isView={state.isView}
          permission={permission}
          onSecurityGroupChange={handleSecurityGroupChange}
        />
      )}
      <Modal
