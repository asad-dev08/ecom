...
import ShippingChargeListHeader from "./ShippingChargeListHeader";
import CreateShippingCharge from "../create-shipping-charge/CreateShippingCharge";
import {
  deleteShippingCharge,
  getShippingCharge,
  getShippingChargesWithPagination,
} from "../../../store/shipping-charge/shippingChargeSlice";
import { getPermissionsForMenu } from "../../../utils/helper";

const ShippingChargeList = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 100,
      render: (isDefault) => (
        <span style={{ color: isDefault ? "#52c41a" : "#595959" }}>
          {isDefault ? "Yes" : "No"}
        </span>
      ),
      filterable: true,
  ...
  const actionHandlers = {
    handleMenuClick: async (e, row) => {
      if (e.key === "edit" || e.key === "view") {
        try {
          const response = await dispatch(getShippingCharge(row.id)).unwrap();
          if (response?.data) {
            setState((prev) => ({
              ...prev,
    ...
    handleDeleteModalOk: async () => {
      try {
        const response = await dispatch(
          deleteShippingCharge(state.selectedRow.id)
        ).unwrap();
        if (response?.statusCode === 200) {
          toast.success(response.message || "Shipping charge deleted successfully");
          setState((prev) => ({
            ...prev,
            deleteModalVisible: false,
            selectedRow: null,
        ...
        toast.error(error.message || "Failed to delete shipping charge");
      }
    },
  ...
  const handleShippingChargeChange = useCallback(() => {
    setState((prev) => ({ ...prev, refreshKey: prev.refreshKey + 1 }));
  }, []);

  return (
    <div>
      <ShippingChargeListHeader
